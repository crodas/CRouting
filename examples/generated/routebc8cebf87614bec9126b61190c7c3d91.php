...
function routebc8cebf87614bec9126b61190c7c3d91($url) {
	...
	switch ($length) {
		...
		case 1:
			if (((($offset_0_1 = strpos($parts[0],'-',0)) !== false) AND (($offset_0_3 = strpos($parts[0],'.',$offset_0_1 + 1)) !== false) AND (($value_0_0 = substr($parts[0],0,$offset_0_1))) AND is_numeric($value_0_0) AND (($value_0_2 = substr($parts[0],1 + $offset_0_1,$offset_0_3 - (1 + $offset_0_1)))) AND ($value_0_4 = substr($parts[0],1 + $offset_0_3) OR ($value_0_4 = 'php') !== false) AND ($offset_0_3 == false OR ('php' == $value_0_4) OR ('html' == $value_0_4) OR ('js' == $value_0_4)))) {
				/* /{foo}-{bar}.{ext} */
				return(array('action' => 'index', 'foo' => $value_0_0, 'bar' => $value_0_2, 'ext' => $value_0_4));
			}
			if ((preg_match('/^[a-zA-Z][a-zA-Z0-9]+$/',$parts[0]))) {
				/* /{controller}/{action} */
